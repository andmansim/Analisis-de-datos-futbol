...
import pandas as pd
from crear_csv1 import actualizar_datos
def regresion ( df_partidos):
    ...
    df_partidos.loc[df_partidos['puntos_local'] < df_partidos['puntos_visitante'], 'resultado'] = int(2) #visitante
    df_partidos.loc[df_partidos['puntos_local'] == df_partidos['puntos_visitante'], 'resultado'] = int(3) #empate
    
    print(df_partidos['resultado'])
    
    #separamos las variables independientes y dependientes
    X = df_partidos[['puntos_local', 'puntos_visitante']]
...
#leemos el csv
df_partido = pd.read_csv('csvs1/partidos_fut_regresion.csv', encoding='utf-8', sep=';')
...
df_partido.to_csv('csvs1/partidos_fut_ronda1.csv', index=False, sep=';')
...
for i in range(len(df_partidos_ronda4)):
    if df_partidos_ronda4['puntos_local'][i] < df_partidos_ronda4['puntos_visitante'][i]:
...
df_partidos_ronda4.to_csv('csvs1/partidos_fut_ronda4.csv', index=False, sep=';')
